...
function findIndexByProperty(arr, name) {
  if (Array.isArray(arr)) {
    const elemIndex = arr.findIndex(elem => elem.name === name)
    return elemIndex;
  }
...
function findLastEvenNumber(arr) {
  if (Array.isArray(arr) && arr.length > 0) {
    const lastElem = arr.findLast(elem => elem % 2 === 0);
    return lastElem || 'Парних чисел не знайдено';
  } else {
...
function getPositiveArrayValues(arr) {
  if (Array.isArray(arr)) {
    ...
    let newArr = Array.from(iterator);
    let result = newArr.filter(elem => elem > 0);
    return result;
  } else {
    return 'Вхідний параметр має бути масивом';
  }
...
function removeAndDouble(arr, startIdx) {
  if (Array.isArray(arr)) {
    arr.splice(startIdx);
    let mapArr = arr.map(elem => elem * 2);
    return mapArr;
  } else {
    return 'Вхідний параметр має бути масивом';
  }
...
function sumAndPrint(arr) {
  if (Array.isArray(arr)) {
    let elemSum = arr.reduce((accumulator, currentValue) => accumulator + currentValue, 0);
...
function flattenSquareAndFilter(arr, threshold) {
  if (Array.isArray(arr)) {
    ...
    let newArr = flatArr.map(elem => elem * elem);
    let result = newArr.filter(elem => elem > threshold);
    return result;
  } else {
    return 'Вхідний параметр має бути масивом';
  }
...
function getLastElementsSumAndPrint(arr, n) {
  if (Array.isArray(arr)) {
    let sliceArr = arr.slice(-n);
    let sumSliceElem = sliceArr.reduceRight((accumulator, currentValue) => accumulator + currentValue, 0);
...
function expandArrayByN(arr, n) {
  if (Array.isArray(arr)) {
    const expandedArray = arr.flatMap((elem) => Array.from({ length: n }, () => elem));
...
function findLongestWord(arr) {
  if (Array.isArray(arr)) {
    const longestWord = arr.reduce((accumulator, currentValue) => {
      return currentValue.length < accumulator.length ? accumulator : currentValue;
    }, 0)
    return longestWord;
  }
...
function capitalizeWords(arr) {
  if (Array.isArray(arr)) {
    const capitalizedArray = arr.map((elem) => elem.charAt(0).toUpperCase() + elem.slice(1))
...
function calculateTotalPrice(arr) {
  if (Array.isArray(arr)) {
    const totalPrice = arr.reduce((accumulator, currentValue) => currentValue.price + accumulator, 0);
    return totalPrice;
  } else {
    return 0;
  }
